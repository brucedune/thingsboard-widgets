...
**9/17 — Aurora 47 (70268923) resolved.** Not a firmware sign inversion: the meter was pulled from Rustic Acres 44 on 9/2 (register froze at 61,883; replacement 79464317 installed 10:46) and reinstalled at Aurora 47 on 9/11 06:50 mounted the other way (two PIN-reset boots, Install/Move Date stamped). Its `waterFlowDir=2` was learned at Rustic and travelled with it -> under-billed 9/11-9/17. Bruce set `waterFlowDir=1`. Lessons: (1) a moved meter must have its direction pin cleared and be re-learned at the new pipe; (2) TB audit log holds no record of widget attribute/group changes - the register freeze and attribute timestamps were the only trail; (3) draw-sign "inversions" across the crossing need a service-event check (Move/Install Date, PIN-reset boots, frozen register) before any firmware blame.

**9/17 — "Claude keeps crashing" diagnosed:** not a crash. The desktop app ran a stealth self-update after an idle timeout three times (9/15 13:32, 9/16 22:56, 9/17 13:28), killing its PTY trees; my nohup background jobs under that PTY hung `conhost.exe` (Event 1002 each time), which locked files and blocked the relaunch until a reboot. Agent, tracker loop and watcher now run DETACHED (`Start-Process -WindowStyle Hidden`); `stepB_loop.py` replaces the .sh loop. The three unsupervised gaps (9/15 00:29 Windows Update restart, 9/15 15:01 and 9/17 00:15 shutdowns, 9/17 14:02 restart) cost no pins and no data.
