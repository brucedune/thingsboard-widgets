...
Billing size: sessions during flow = event-open hourly reports, event-end (the last ~50 s of every event is also lost when the session starts at event end — no, that one starts after flow stops, so only the tail matters), check-ins landing mid-flow, recal sessions. ~45-60 s per session; on an hourly open-event cadence that is ~1.5% of a long event; at this rig's 12 gpm ~10 gal per session. Fleet: every Gen2 on a TIUART_ONLY build with a UART-only TI (v305+), i.e. the whole current fleet.
